...
async def log_tagged_messages(event):
    ...
    resalt = f"#الـتاك \n<b>الـكروب : </b><code>{hmm.title}</code>"
    if full is not None:
        resalt += (
            f"\n<b>الـمرسل : </b> 👤{_format.htmlmentionuser(full.first_name , full.id)}"
        )
    if messaget is not None:
        resalt += f"\n<b>نـوع الرسـالة : </b><code>{messaget}</code>"
    else:
        resalt += f"\n<b>الـرسالة : </b>{event.message.message}"
    resalt += f"\n<b>رابـط الـرسالة: </b><a href = 'https://t.me/c/{hmm.id}/{event.message.id}'> هـنا الرابط</a>"
...
@jmthon.ar_cmd(
    pattern="ك تسجيل (on|off)$",
    command=("ك تسجيل", plugin_category),
    info={
