...
async def catgban(event):
    ...
    if user.id == (await event.client.get_me()).id:
        await cate.edit("لا استـطيع حـظر نفسي 🌿⚕️")
        return
    if user.id in CAT_ID:
        await cate.edit("لا استطيع حظر مطوري ⚕️🌿")
        return
    try:
...
async def catgban(event):
    if event.fwd_from:
        return
    cate = await edit_or_reply(event, "__جـار الـغاء الحـظر ⚕️__")
    start = datetime.now()
    user, reason = await get_user_from_event(event, cate)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        ...
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) لـيس موجـود في قـائمه الحـظر اصلا 🙁💞"
        )
        return
    san = []
    san = await admin_groups(event)
    ...
    if len(gbanned_users) > 0:
        for a_user in gbanned_users:
            if a_user.reason:
                ...
                GBANNED_LIST += (
                    f"👉 [{a_user.chat_id}](tg://user?id={a_user.chat_id}) بدون سبب\n"
                )
    else:
        GBANNED_LIST = "لايوجد محـظورين حتتى الان 🙁💞"
...
async def startgmute(event):
    ...
    if event.is_private:
        ...
        if not user:
            return
        if user.id == bot.uid:
            return await edit_or_reply(event, "لا استطيع كتم نفسي 😕⚕️")
        userid = user.id
    try:
    ...
    if is_muted(userid, "gmute"):
        return await edit_or_reply(
            event,
            f"هذًا الـمستخدم مـكتوم بالـفعل ⚕️💞",
        )
    try:
        mute(userid, "gmute")
        ...
        if reason:
            ...
            await edit_or_reply(
                event,
                f"{_format.mentionuser(user.first_name ,user.id)} ▪️︙ تم كتمه من هن بنجاح ✅",
            )
    if BOTLOG:
        reply = await event.get_reply_message()
        if reason:
            await event.client.send_message(
                BOTLOG_CHATID,
                "#المكتومين\n"
                f"**المستخدمين :** {_format.mentionuser(user.first_name ,user.id)} \n"
                f"**السبب :** `{reason}`",
            )
        else:
            await event.client.send_message(
