...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        ...
        await edit_delete(args, "**لا يمكنني اخذ هذا **")
        return
    if photo:
        ...
        if (
            ...
            await edit_delete(
                ...
                f"تـم اخذ الملصق بنـجاح\
                    \n 𖥻 حزمة ملصقاتك هنا  [اضغط هنا](t.me/addstickers/{packname}) **",
                parse_mode="md",
                time=10,
            )
        else:
            ...
            if otherpack:
                await edit_delete(
                    catevent,
                    f"𖥻 تم صنع الملصق لحزمه مختلفه !\
                    \n𖥻 والحزمة التي تم إنشاؤها حديثًا هي` [اضغط هنا](t.me/addstickers/{packname}) `  استخدم  {emoji}` للعثور على الملصقات المصنوعه ",
                    parse_mode="md",
                    time=10,
                )
            else:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    try:
        ...
        return await edit_delete(
            catevent,
            " أعتقد أن هذا الملصق ليس من أي حزمة. لذا ، لا أستطيع أخذ هذا الملصق الى حزمتي",
        )
    kangst = 1
...
async def get_pack_info(event):
    ...
    try:
        ...
        catevent = await edit_or_reply(
            event, "** يتم سحب معلومات الملصق الرجاء الانتظار 𖥻**"
        )
    except BaseException:
        return await edit_delete(
            event, "**- هذا ليس ملصق يجب الرد على الملصق اولا**", 5
    ...
    OUTPUT = (
        f"**𖥻 عنوان الملصق:** `{get_stickerset.set.title}\n`"
        f"**𖥻 الاسم القصير للملصق:** `{get_stickerset.set.short_name}`\n"
        f"**𖥻 المـسؤل:** `{get_stickerset.set.official}`\n"
        f"**𖥻 الارشيف:** `{get_stickerset.set.archived}`\n"
        f"**𖥻 حزمة الملصق:** `{get_stickerset.set.count}`\n"
        f"**𖥻 الايموجي المستخدم:**\n{' '.join(pack_emojis)}"
    )
