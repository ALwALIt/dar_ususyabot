    ...
    if reply:
        if input_str and input_str.isnumeric():
            ...
            return await edit_or_reply(
                event, f"**خـطأ**\n`{input_str} الرجاء الكتابة بشكل صحيح"
            )
        else:
            async for msg in event.client.iter_messages(
    ...
    if msg_src:
        if input_str and input_str.isnumeric():
            ...
            try:
                ...
                if BOTLOG:
                    await event.client.send_message(
                        BOTLOG_CHATID, "#المـسح \n** تم حذف الرسالة بنجاح ✅**"
                    )
            except rpcbaseerrors.BadRequestError:
                if BOTLOG:
                    await event.client.send_message(
                        BOTLOG_CHATID,
                        "** لا يمكنني الحذف احتاج صلاحيات ادمن**",
                    )
        elif input_str:
            ...
            try:
                ...
                await edit_or_reply(event, "** لا يمـكنيي حذف الـرسالة**")
    elif not input_str:
        await event.delete()
