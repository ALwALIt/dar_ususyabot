...
def main_menu():
    ...
    buttons = [
        ...
        (
            ...
            Button.inline(
                f"🤖 الـبوت ({len(GRP_INFO['bot'])})",
                data=f"bot_menu",
            ),
        ),
        (
            Button.inline(
                f"🎨 الـتسلية ({len(GRP_INFO['fun'])})",
                data=f"fun_menu",
            ),
            Button.inline(
                f"🧩 مـتنوعة ({len(GRP_INFO['misc'])})",
                data=f"misc_menu",
            ),
        ),
        (
            ...
            Button.inline(
                f"🗂 الـربط ({len(GRP_INFO['utils'])})",
                data=f"utils_menu",
            ),
        ),
        (
            Button.inline(
                f"➕ الاخـرى ({len(GRP_INFO['extra'])})",
                data=f"extra_menu",
            ),
            Button.inline(
                f"🔒 اغـلق الـقائمة",
...
async def on_plugin_callback_query_handler(event):
    text = f"الـملف: {len(PLG_INFO)}\
        \nالأمـر: {len(CMD_INFO)}\
        \n\n{tr}مساعدة <الملف> : لمعلومات الملف .\
        \n{tr}مساعدة -𝚌 <الامر> : 𝙵𝚘𝚛 𝚊𝚗𝚢 𝚌𝚘𝚖𝚖𝚊𝚗𝚍 𝚒𝚗𝚏𝚘.\
        \n{tr}𝚜 <المطلوب> : 𝚃𝚘 𝚜𝚎𝚊𝚛𝚌𝚑 𝚊𝚗𝚢 𝚌𝚘𝚖𝚖𝚊𝚗𝚍𝚜.\
        "
