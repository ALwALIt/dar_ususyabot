...
async def _(event):
    ...
    if Config.CHROME_BIN is None:
        return await edit_or_reply(
            event, "-** يجب تنزيل متصفح كوكل كروم اولا ⚕️🌿"
        )
    ...
    try:
        ...
        if not caturl:
            ...
            caturl = url(inputstr)
        if not caturl:
            return await catevent.edit("__الـرابط المعـطى خطأ__")
        driver.get(inputstr)
        await catevent.edit("__يتم حسب قياسات الصورة__")
        ...
        await catevent.edit("**متصفح كروم متوقف**")
        driver.close()
