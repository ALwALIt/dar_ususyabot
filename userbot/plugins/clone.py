...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "يفشل الأمر حين يتحول إلى واجب بدلًا من رغبة  - @JMTHON"
)
...
async def _(event):
    "يجب الرد على المستخدم لنسخ حسابه بالكامل"
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#الانتحال\nتم بنجاح الانتحال [{first_name}](tg://user?id={user_id })",
        )
...
async def _(event):
    ...
    await edit_delete(evenr, "**تك اعادة الحساب بنجاح ✅**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"#الاعادة\nتم ارجاع الحساب الى وضعه الطبيعي بنجاح ✅"
        )
