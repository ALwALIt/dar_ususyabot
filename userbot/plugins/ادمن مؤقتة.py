...
async def tmuter(event):  # sourcery no-metrics
    ...
    catevent = await edit_or_reply(event, "** يتم الكتم 🧸♥**")
    user, reason = await get_user_from_event(event, catevent)
    if not user:
        return
    if not reason:
        return await catevent.edit("** انت لم تقم بوضع وقت مع الامر ارسـل  للميزد من المعلومات")
    ...
    if user.id == event.client.uid:
        return await catevent.edit(f"** عـذرا لا يمكنني حظـر نفسي **")
    try:
...
async def tban(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**- يبدو انك لم تقم بوضع وقت مع الامر ارسل **")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    cattime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
