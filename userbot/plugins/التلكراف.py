...
@jmthon.ar_cmd(
    pattern="(ت(ل)?ك(راف)?) ?(m|t|media|text)(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    catevent = await edit_or_reply(event, "⌔︙ يتم المعالجة ")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"⌔︙ تـم صنع حساب تيـليكراف جديد {auth_url} للعـملية الحالية. \n⌔︙ لا تعطي هذا الرابط لاي شخـص حتى لو قـال هو من الـمطورين",
        )
    optional_title = event.pattern_match.group(5)
    if not event.reply_to_msg_id:
        return await catevent.edit(
            "⌔︙ يـجب الرد على الرسالة اولا للحصول على رابـط تلكراف ",
        )
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await catevent.edit(f"⌔︙ تـم التحميـل الى {downloaded_file_name}")
        ...
        try:
            ...
            os.remove(downloaded_file_name)
            await catevent.edit(
                f"⌔︙ الـرابط : [تـلكراف](https://telegra.ph{media_urls[0]})\
                    \n⌔︙ الـوقت الـمستغرق : `{ms} من الـثواني.`",
                link_preview=True,
            )
        ...
        await catevent.edit(
            f"⌔︙ الرابـط :  [تلـكراف]({cat})\
                 \n⌔︙ الـوقت المستـغرق : `{ms} من الثواني.`",
            link_preview=True,
        )
