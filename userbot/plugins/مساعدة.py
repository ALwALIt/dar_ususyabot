...
async def plugininfo(input_str, event, flag):
    ...
    if len(cmds) == 1 and (flag is None or (flag and flag != "-p")):
        ...
        return outstr
    outstr = f"**الـملف : **`{input_str}`\n"
    outstr += f"**الأوامـر المتاحـة :** `{len(cmds)}`\n"
    category = getkey(input_str)
    if category is not None:
        outstr += f"**الـفئة :** `{category}`\n\n"
    for cmd in cmds:
        outstr += f"•  **الأمر :** `{cmdprefix}{cmd}`\n"
        try:
            outstr += f"•  **معلومـات :** `{CMD_INFO[cmd][1]}`\n\n"
        except IndexError:
            outstr += f"•  **معلومـات :** `None`\n\n"
    outstr += f"**👩‍💻 الأسـتخدام : ** `{cmdprefix}help <command name>`\
        \n**Note : **If command name is same as plugin name then use this `{cmdprefix}help -c <command name>`."
    return outstr


async def grpinfo():
    outstr = "**اوامـر بوت الـقط هي:**\n\n"
    outstr += f"**👩‍💻 الاستخدام : ** `{cmdprefix}help <plugin name>`\n\n"
...
async def cmdlist():
    ...
    for cat in category:
        ...
        for plugin in plugins:
            ...
            outstr += "\n"
    outstr += f"**👩‍💻 الأستـخدام : ** `{cmdprefix}help -c <command name>`"
    return outstr
