...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#الأنـتحال\nتم النـسخ بنجـاح  [{first_name}](tg://user?id={user_id })",
        )
...
async def _(event):
    ...
    await edit_delete(event, "**تم بنجاح اعادة حسابك ✅**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"#الأعـادة\nتم بـنجاح اعادة حسابك الى وضعه الطبيعي**"
        )
