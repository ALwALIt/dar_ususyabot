...
# Translate into Arabic by  : @RRRD7
import asyncio
import base64
import os
import random
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        fnt = ImageFont.truetype(cat, 122)
...
async def bloom_pfploop():
    ...
    while BLOOMSTART:
        ...
        drawn_text.text((95, 250), "      𖥻", font=ofnt, fill=(FR, FG, FB))
        img.save(autophoto_path)
        file = await catub.upload_file(autophoto_path)
        try:
            await catub(functions.photos.UploadProfilePhotoRequest(file))
...
async def autopfp_start():
    if gvarstatus("strings_autopic") is not None:
        AUTOPFP_START = True
        string_list = COLLECTION_STRINGS[gvarstatus("strings_autopic")]
    else:
        AUTOPFP_START = False
    i = 0
    while AUTOPFP_START:
        ...
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOPFP_START = gvarstatus("strings_autopic") is not None
...
async def _(event):
    ...
    if gvarstatus("autopic_counter") is not None and gvarstatus("autopic") == "true":
        return await edit_delete(event, f" الصورة الوقتية بالتاكيد مفعلة**")
...
@catub.cat_cmd(
    pattern="صورة_وقتية$",
    command=("صورة_وقتية", plugin_category),
    info={
...
async def _(event):
    ...
    if gvarstatus("digitalpic") is not None and gvarstatus("digitalpic") == "true":
        return await edit_delete(event, f"** الصورة الوقتية بالفعل مُمكـنة ❕** ")
    addgvar("digitalpic", True)
    await edit_delete(event, f"** تم تفعيل الصـورة الوقتية بنـجاح ✅**")
...
async def _(event):
    ...
    if gvarstatus("بايو وقتي") is not None and gvarstatus("بايو وقتي") == "true":
        return await edit_delete(event, f"** البايو الوقتي بالفعل مُمكـن ❕**")
    addgvar("بايو وقتي", True)
...
async def _(event):  # sourcery no-metrics
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "thorpfp" and gvarstatus("strings_autopic") is not None:
        pfp_string = gvarstatus("strings_autopic")[:-8]
    ...
    if input_str == "batmanpfp" and gvarstatus("strings_autopic") is not None:
        pfp_string = gvarstatus("autopfp_strings")[:-8]
    ...
    if input_str == "صورة_وقتية":
        if gvarstatus("digitalpic") is not None and gvarstatus("digitalpic") == "true":
            ...
            return await edit_delete(event, "** تم ايقاف الصـورة بنجـاح  **✅")
        return await edit_delete(event, "** الصورة الوقتية لم تفعل اصلا ❕**")
    if input_str == "bloom":
        if gvarstatus("bloom") is not None and gvarstatus("bloom") == "true":
            delgvar("bloom")
            if os.path.exists(autopic_path):
    ...
    if input_str == "اسم وقتي":
        if gvarstatus("اسم وقتي") is not None and gvarstatus("اسم وقتي") == "true":
            ...
            return await edit_delete(event, "** تم ايقاف الاسم الوقتي بنجـاح **✅")
        return await edit_delete(event, "الاسم الوقتي لم يفعل اصلا ❕**")
    if input_str == "بايو وقتي":
        if gvarstatus("بايو وقتي") is not None and gvarstatus("بايو وقتي") == "true":
            ...
            return await edit_delete(event, "** تم تعطيل البـايو الوقـتي بنـجاح ✅**")
        return await edit_delete(event, "** لم يتـم تفعيل البايو الوقتي اصلا ❕**")
    END_CMDS = [
        "autopic",
        "صورة_وقتية",
        "bloom",
    ...
    if input_str not in END_CMDS:
        await edit_delete(
            event,
            f"{input_str} \n**هـذا خطا يجب كتابة الامر بشكل صحيح اولا**",
            parse_mode=_format.parse_pre,
        )
