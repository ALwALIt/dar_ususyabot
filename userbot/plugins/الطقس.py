...
async def get_tz(con):
    """ ⌔︙رجـاٍ حط منطقه زمينه خاصه بيك مثلا :Asia/Baghdad من الفارات الخاصه بك. """
    """ . """
    for c_code in c_n:
        if con == c_n[c_code]:
...
async def get_weather(weather):
    """ ⌔︙ يـرجئ تتـعـين منطقه زمـنيه خـاصه بك. """

    if not OWM_API:
        await weather.edit(
            "`احصل ع الـمنطقه الزمنيه منا ` https://openweathermap.org/ `first.`")
        return
    ...
    if not weather.pattern_match.group(1):
        ...
        if not CITY:
            await weather.edit(
                "`⌔︙يجب تعين منطقه من الفارات كمنطقه افتراضيه لك.`"
            )
            return
    else:
