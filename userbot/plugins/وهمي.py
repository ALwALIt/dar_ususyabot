...
async def _(event):
    ...
    if len(args) == 0:
        ...
        await edit_delete(event, "**خطا **")
        return
    try:
        if scam_time > 0:  #Arabiccat by  @RRRD7.- @UUNZZ
            await event.delete()
...
async def _(event):
    ...
    await catevent.edit("** تم اعطاء لقب بنجـاح ✅**")


@catub.cat_cmd(
    pattern="بادمن$",
    command=("بادمن", plugin_category),
    info={
...
async def _(event):
    ...
    event = await edit_or_reply(event, "** يتم الرفع")
    animation_chars = [
        "**جـاري رفع مشرف...**",
        "**يتم اعطاء صلاحيات للمستخدم...**",
        "**(1) إرسل رسائل: ☑️**",
        "**(1) إرسل رسائل: ✅**",
        "**(2) إرسال الوسائط: ☑️**",
        "**(2) إرسال الوسائط: ✅**",
