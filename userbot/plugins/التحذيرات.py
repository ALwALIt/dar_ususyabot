...
async def _(event):
    ...
    if num_warns >= limit:
        ...
        if soft_warn:
            logger.info("الأمـر: طـرد المسـتخدم")
            reply = "{} التحـذيرات, [المستخـدم](tg://user?id={}) تـم طـرده بنـجاح ✅".format(
                limit, reply_message.sender_id
            )
        else:
            logger.info("TODO: ban user")
            reply = "{} التحـذيرات, [المستخـدم](tg://user?id={}) تـم حظـره بنـجاح ✅!".format(
                limit, reply_message.sender_id
            )
    else:
...
async def _(event):
    ...
    reply_message = await event.get_reply_message()
    sql.reset_warns(reply_message.sender_id, event.chat_id)
    await edit_or_reply(event, "** تم حذف جميع الـتحذيرات لـهذا الشخص**")
